...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof dnanasumors.ndarray, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( dnanasumors.ndarray.length, 4, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the sum of absolute values (ignoring NaN values)', function test( t ) {
	...
	v = dnanasumors.ndarray( x.length, x, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `0`', function test( t ) {
	...
	v = dnanasumors.ndarray( 0, x, 1, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );

	v = dnanasumors.ndarray( -1, x, 1, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );
...
tape( 'if provided a `stride` parameter equal to `0`, the function returns the sum of the first element repeated N times', function test( t ) {
	...
	v = dnanasumors.ndarray( x.length, x, 0, 0 );
