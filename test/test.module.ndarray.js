...
tape( 'a module instance has an `ndarray` method which calculates the sum of strided array elements (ignoring NaN values)', function test( t ) {
	var mem;
	var mod;
	var xp;
	var y;
	...
	mod.write( xp, new Float64Array( [ 1.0, -2.0, -4.0, 5.0, NaN, 0.0, 3.0, 0.0, -3.0, 3.0 ] ) ); // eslint-disable-line max-len

	y = mod.ndarray( 10, xp, 1, 0 );
	t.strictEqual( y, 21.0, 'returns expected value' );
...
tape( 'a module instance has an `ndarray` method which supports a negative `stride` parameter', function test( t ) {
	...
	y = mod.ndarray( N, xp, -2, 8 );
	t.strictEqual( y, 9.0, 'returns expected value' );

	t.end();
});

tape( 'if provided a `stride` parameter equal to `0`, a module instance has an `ndarray` method which returns the sum of the first element repeated N times', function test( t ) {
	var mem;
	var mod;
	var xp;
	var y;
